...
import { Plus, Trash2, Server, Eye, EyeOff, Edit2, Check, X, Loader2, KeyRound } from "lucide-react";
...
export function ProvidersTab({ providers, addProvider, removeProvider, updateProvider, startPollinationsOAuth, pollinationsLoading, pollinationsError }: ProvidersTabProps) {
  ...
  const handleAdd = () => {
    ...
    addProvider({
      id,
      name: customName || preset || "Custom Provider",
      type: customType,
...
function ProviderEditForm({
  ...
  const handleSave = () => {
    const envVar = apiKey
      ...
      : '';
    onSave({ name, baseUrl, apiKeyEnvVar: envVar, apiKey, defaultModel });
  };
    ...
    <div className="space-y-2">
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <Input className="h-7 text-xs font-mono" value={defaultModel} onChange={(e) => setDefaultModel(e.target.value)} />
        </div>
      </div>
      <div>
        <Label className="text-xs text-muted-foreground">Base URL</Label>
        <Input className="h-7 text-xs font-mono" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} />
      </div>
      <div>

...
export default function Home() {
  ...
  const handleSave = async () => {
    ...
    const hostsRaw = params.get("hosts");

    // No server params — fall back to copy dialog immediately
    if (!port || !hostsRaw) {
      setSaveOpen(true);
      return;
    }

    const hosts = hostsRaw.split(",").filter(Boolean);
    if (hosts.length === 0) {
      setSaveOpen(true);
      return;
    }
        ...
        <div className="flex w-full min-h-0 flex-7 flex-col gap-4 lg:w-2/5">
          ...
          >
            <div className="flex h-full flex-col overflow-hidden">
              <div className="shrink-0 border-b border-border px-4 py-4 md:px-5">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="text-lg font-semibold tracking-tight">网络与扩展</h2>
                  <Button
                    type="button"
                    size="sm"
                    variant="ghost"
